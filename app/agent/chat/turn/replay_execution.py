...
async def run_chat_replay_execution(
    *,
    request_id: str,
    conversation_id: int,
    turn_id: str,
    ...
    """独立于 HTTP 连接执行 Chat turn，并将业务帧写入 Redis replay log。"""
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "chat.turn.replay_shutdown_frame_failed",
                request_id=request_id,
                turn_id=turn_id,
                error=str(exc),
            )
        ...
        logger.exception(
            "chat.turn.replay_execution_failed",
            request_id=request_id,
            turn_id=turn_id,
            error=str(exc),
        )
    ...
    *,
    ...
    while not cancel_event.is_set():
        ...
        try:
            ...
            logger.exception("chat.turn.replay_lease_refresh_failed", request_id=request_id, error=str(exc))
            lease_lost_event.set()
            cancel_event.set()
            return
        if not refreshed:
            logger.error("chat.turn.replay_lease_lost", request_id=request_id)
            lease_lost_event.set()
            cancel_event.set()
            return
        try:
            lock_refreshed = await conversation_turn_lock.refresh(conversation_id, turn_id)
        except Exception as exc:
            logger.exception(
                "chat.turn.conversation_lock_refresh_failed",
                request_id=request_id,
                conversation_id=conversation_id,
                turn_id=turn_id,
                error=str(exc),
        ...
        if not lock_refreshed:
            logger.error(
                "chat.turn.conversation_lock_lost",
                request_id=request_id,
                conversation_id=conversation_id,
                turn_id=turn_id,
            )
