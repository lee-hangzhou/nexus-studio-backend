...
from app.server.canvas.services.errors import INVALID_NODE_ID
...
async def _list_generations(project_id: int, user_id: int, args: ListNodeGenerationsInput) -> ToolResult:
    ...
    task_ids: list[int] = []
    if args.node_id:
        node = await get_canvas_port().get_node(project_id, args.node_id)
        if node is None:
            return ToolResult.fail(INVALID_NODE_ID, detail=args.node_id)
        if node.task_id is not None:
            task_ids.append(node.task_id)
    else:
        task_ids = await get_canvas_port().list_project_node_task_ids(project_id, limit=args.limit)
    ...
    for task_id in task_ids[: args.limit]:
        try:
            ...
            view = await get_generation_port().assemble_task_view(observed, user_id)
