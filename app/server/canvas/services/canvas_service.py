...
def _asset_allowed_for_node(scope: CanvasScope, node: CanvasNodeView, asset: Assets) -> bool:
    ...
    metadata = (
        ...
        else _JSON_OBJECT.validate_python(asset.metadata)
    )
    episode_id = _metadata_int(metadata, "episode_id")
    if episode_id is not None:
        return episode_id == scope.episode_id

    task_id = node.data.generate_task_id
    if task_id is not None and asset.source_type == ASSET_SOURCE_GENERATE_RESULT:
...
async def refresh_node_asset_urls(nodes: list[CanvasNodeView], *, scope: CanvasScope) -> None:
    ...
    asset_ids = {
        int(asset_id)
        for node in nodes
        for asset_id in (node.data.output_asset_ids or [])
    }
    assets = (
        await Assets.filter(
        ...
        for row in reversed(rows):
            ...
            views.append(
                CanvasMessageView(
                    ...
                    role=row.role,
                    content=row.content,
                    input=_OPTIONAL_JSON_OBJECT.validate_python(raw_input),
                    metadata=metadata,
        ...
        async with in_transaction():
            ...
            for op in ops:
                if isinstance(op, CreateNodeOp):
                    payload = op.node
                    node_id = uuid4()
                    data = ensure_create_defaults(payload.kind, payload.data)
                    ...
                    payload = op.node
                    row = await self._get_node_for_update(scope.episode_id, payload.id)
                    self._require_entity_revision(row, payload.revision, kind="node", entity_id=payload.id)
                    if payload.position is not None:
                        row.position_x = payload.position.x
                        row.position_y = payload.position.y
                    ...
                    if payload.data is not None:
                        ...
                        merged = merge_client_node_data(existing, payload.data, kind=row.kind)
    ...
    async def _collect_patch_conflicts(
        ...
        for op in ops:
            if isinstance(op, UpdateNodeOp):
                node_id = op.node.id
                expected = op.node.revision
                if node_id in deleted_nodes:
                    raise AppError(ErrorCode.RESOURCE_NOT_FOUND, f"node {node_id} not found")
                if node_id not in node_working:
                    row = await self._get_node_for_update(episode_id, node_id)
                    node_working[node_id] = row.revision
                actual = node_working[node_id]
                if actual != expected:
                    conflicts.append(
                        CanvasRevisionConflictItem(
                            kind="node",
    ...
    async def update_node_generation(
        ...
        *,
        ...
        async with in_transaction():
            ...
            existing = parse_node_data(row.data)
            merged = apply_generation_to_data(
                existing,
                status=status,
                generate_task_id=task_id,
                generate_error=error_message,
                output_asset_ids=attached_ids,
                model=model_id,
                voice_id=voice_id,
    ...
    async def update_node_text_output(
        ...
        *,
        ...
        async with in_transaction():
            ...
            payload["generate_task_id"] = None
            if output_text is not None:
                payload["content"] = output_text
            if error_message is not None:
                payload["generate_error"] = error_message
