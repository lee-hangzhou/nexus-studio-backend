...
Do **not** write placeholder sentences like「根据上游提示词与参考图…」into the video/image node's `input_prompt` as the final submit text. The fused prompt goes in `submit_node_generation.prompt`, not as a lazy template on the node.
...
- Create a text node with the user's request in `input_prompt` and the generated final prompt in `output_text`.
- Connect `text.output_text` to `image.prompt_input` only after both nodes exist; use real `nodes[].id` from a prior patch (see `canvas_operations` — two-step create then connect).
- Submit image generation with your fused prompt and resolved refs.
- Connect `image.output_asset` to `video.reference_asset`; the workflow runner continues downstream after the image asset is ready (fills missing `model_id` / `duration_sec` from gateway model list when the video node row omits them).
- Do not invent asset ids or signed URLs. Use real `output_asset_ids` returned by tools or node queries.
